...
Options:
  ...
  --backend NAME      auto, native, libsecret, or none
  --build-dir PATH    Build directory (default: build/linux-<config>)
...
case "$credential_backend" in
  auto|native|libsecret|none) ;;
  *) echo "Invalid Linux credential backend: $credential_backend" >&2; exit 2 ;;
esac
...
if [[ -z "$build_directory" ]]; then
  build_directory="$repo_root/build/linux-$configuration_lower"
...
echo "Configuring Linux build ($configuration, backend=$credential_backend)..."
